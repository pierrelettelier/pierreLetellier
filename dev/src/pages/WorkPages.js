...
export default function WorkPage() {
  ...
  const [mansoryData, setMansoryData] = useState(null)
...
`
    const mansoryQuery = `
  *[_type == "Mansory" && (slug.current == $slug || title == $slug)][0]{
...
`
    ...
    // Requête de débogage pour voir tous les documents Mansory
    const debugMansoryQuery = `*[_type == "Mansory"]{title, slug}`
  ...
  `
    ...
    // Première requête de débogage pour voir tous les documents Mansory
    client.fetch(debugMansoryQuery)
      .then(allMansory => {
        console.log('🔍 Tous les documents Mansory disponibles:', allMansory)
      })

    Promise.all([
      client.fetch(workQuery, { slug }),
      client.fetch(headerQuery, { slug }),
      client.fetch(mansoryQuery, { slug })
    ])
      .then(([workRes, headerRes, mansoryRes]) => {
        console.log('🔍 Slug utilisé:', slug)
        console.log('🔍 Données mansory reçues:', mansoryRes)
        setWorkData(workRes)
        setHeaderData(headerRes)
        setLoading(false)
        setMansoryData(mansoryRes)
      })
      .catch((error) => {
        console.error(error)
        setLoading(false)
  ...
  // 🔥 Fonction pour ouvrir modal (image360, youtubeUrl ou mainImage)
  const handleOpenModal = (work) => {
    if (work.mainImage || work.image360 || work.youtubeUrl) {
  ...
  useEffect(() => {
    if (selectedWork?.image360?.asset?.url && panoContainerRef.current) {
      ...
      const panorama = new PANOLENS.ImagePanorama(selectedWork.image360.asset.url)
  ...
  return (
    <div className="work-page">
      ...
      {/* 🔥 LISTE DES PROJETS */}
      <section >
        {mansoryData?.rows?.map((row) => (
          <div key={row.lineNumber} className="mansory">
            {row.items.map((work, index) => {
              // Analyser la composition de la rangée
              const landscapeCount = row.items.filter(item => item.mainImage.orientation === 'landscape').length;
              const portraitCount = row.items.filter(item => item.mainImage.orientation === 'portrait').length;
              const totalItems = row.items.length;
              
              let width = '33%'; // défaut
              
              // Logique de calcul des largeurs
              if (totalItems === 1) {
                // Une seule image = 100%
                width = '100%';
              } else if (landscapeCount === 2 && totalItems === 2) {
                // 2 landscapes = 50% chacune
                width = '50%';
              } else if (portraitCount === 2 && totalItems === 2) {
                // 2 portraits = 50% chacun
                width = '50%';
              } else if (landscapeCount === 1 && portraitCount === 1) {
                // 1 landscape + 1 portrait = 66% + 33%
                width = work.mainImage.orientation === 'landscape' ? '66%' : '33%';
              } else if (portraitCount === 3) {
                // 3 portraits = 33% chacun
                width = '33%';
              } else if (work.mainImage.orientation === 'landscape') {
                width = '66%';
              } else {
                width = '33%';
              }
              ...
              return (
                <div
                  key={work._id}
                  className={`mansory-item ${work.mainImage.orientation}`}
                  style={{
                    width: width,
                    height: '526px'
                  }}
        ...
        <div className="modal-overlay" onClick={handleCloseModal}>
          <div 
            className={`modal-content ${
              selectedWork.image360?.asset?.url 
                ? 'modal-360' 
          ...
          >
            ...
            {/* Cas 2: Vidéo YouTube */}
            {selectedWork.youtubeUrl && (
              <div className="youtube-container" style={{ width: '100%', height: '80vh' }}>
                <iframe
                  width="100%"
                  height="100%"
                  src={selectedWork.youtubeUrl.replace('watch?v=', 'embed/')}
            ...
            {/* Cas 3: Image classique (par défaut) */}
            {selectedWork.mainImage?.asset?.url && !selectedWork.image360?.asset?.url && !selectedWork.youtubeUrl && (
              <img
                src={selectedWork.mainImage.asset.url}
